...
santillana_etal[["OBI"]] <-
  argo(y,exogen=exogen,
       alpha = 1, N_lag = NULL, use_all_previous = TRUE,  N_training = 12)  #不加时间序列

our_argo[["withoutlag"]] <-
  argo(logit(y/100), exogen = log((exogen + 0.5) / 100),
       alpha = 1, N_lag=1:26, use_all_previous = FALSE, N_training =26)   #argo

ar3 <- argo(y, alpha = NA, use_all_previous = FALSE, N_lag=1:3,
            N_training = 24)  #无惩罚项，无百度指数，结果不稀疏

#####加滞后#######
library(xts)
library(readxl)
Realdata<-read_excel("C:/Users/Administrator/Desktop/Google搜索词.xlsx")
names(Realdata[,1])<-"Date"
for(i in 1:55){
  Realdata[,i+1]<-100*Realdata[,i+1]/max(Realdata[,i+1])  #规范化
}
...
start <- which(colnames(our_argo2$withlag$coef)=="2014-02-01")
end <- which(colnames(our_argo2$withlag$coef)=="2016-05-01")
ARGO_coef <- our_argo2$withlag$coef[, start:end]
...
Saledata_xts<-Saledata_xts["2007-07-01/"]
our_argo$withoutlag$pred<-our_argo$withoutlag$pred["2007-07-01/"]
santillana_etal$OBI$pred<-santillana_etal$OBI$pred["2007-07-01/"]
ar3$pred<-ar3$pred["2007-07-01/"]


pred_xts_blend <- merge(Saledata_xts,logit_inv(our_argo2$withlag$pred)*100*max(Saledata[,2]),logit_inv(our_argo$withoutlag$pred)*100*max(Saledata[,2]),
                        santillana_etal$OBI$pred*max(Saledata[,2]),ar3$pred*max(Saledata[,2]), all=FALSE)


names(pred_xts_blend) <- c("Salesvolume", "ARRGO","ARGO", "Santillana", "AR3")

pred_xts_blend$naive <- c(NA, as.numeric(pred_xts_blend$Salesvolume[-nrow(pred_xts_blend)]))

#pred_xts_blend$ARRGO[zoo::index(our_argo2$withlag$pred)] <- logit_inv(our_argo2$withlag$pred)*100*max(Saledata[,2])

#pred_xts_blend$ARGO[zoo::index(our_argo$withoutlag$pred)] <- logit_inv(our_argo$withoutlag$pred)*100*max(Saledata[,2])

#pred_xts_blend$Santillana[zoo::index(santillana_etal$OBI$pred)] <- santillana_etal$OBI$pred*max(Saledata[,2])


pred_xts_blend <- pred_xts_blend["2014-02-01/"]

model_names <- c("ARRGO","ARGO","Santillana","AR3","naive")
legend_names <- c("ARRGO","ARGO","Santillana_etal","AR(3)","Naive")
...
argo_cut_date <- tail(zoo::index(our_argo2$withlag$pred),1)+1  #只截取最后若干项

setwd("C:/Users/Administrator/Desktop/Baidu all")
pdf("final_plot.pdf", height=11,width=12)
plot_all3(pred_xts_blend, argo_cut_date, model_names[1:5], legend_names[1:5], zoom_periods)
dev.off()
...
rela_effi <- sapply(model_names[-1], function(model_bench)
  bootstrap_relative_efficiency(
   pred_xts_blend, model_names[1], model_bench, l=26, sim = "geom"))
