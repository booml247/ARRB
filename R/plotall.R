...
#'
#' @export
plot_all<- function(GFT_xts, GC_GT_cut_date, model_names, legend_names, zoom_periods){
  par_orig <- par()[c("mar","mfrow")]
  cutting_dates <- which(index(GFT_xts) %in% c(as.Date("2013-01-01"), GC_GT_cut_date-1))#GC_GT_cut_date-94608001
  
  color_code <- c("red","limegreen", "blue", "goldenrod3")
  ...
  xts::plot.xts(GFT_xts$销量数据, ylim=c(-0.1,1.0), type='n', auto.grid = FALSE, main=NA)
  ...
  xts::plot.xts(GFT_xts$销量数据-GFT_xts[,model_names[i]], ylim=c(-1,1),main=NA,typ='n',
  ...
  for(i in length(model_names):1){
    lines(-GFT_xts$销量数据+GFT_xts[,model_names[i]], col=color_code[i],
  ...
  #  plot(GFT_xts$销量数据[plot_period], ylim=c(1.2,8.5),
  #       main=NA, type='n', auto.grid = FALSE)
  #  lines(GFT_xts$销量数据[plot_period], lwd=cdc_lwd)
  #  for(i in length(model_names):1){
