...
library(xts)
library(readxl)
Realdata<-read_excel("C:/Users/Administrator/Desktop/汽车网站搜索统计.xlsx")
...
our_argo[["baidu"]] <-
  argo(logit(y / 100), exogen = log((exogen + 0.5) / 100),
       alpha = 1, N_lag=1:24, use_all_previous = FALSE, N_training = 48)   #argo
...
start <- which(colnames(our_argo$baidu$coef)=="2014-02-01")
end <- which(colnames(our_argo$baidu$coef)=="2015-11-01")
...
pred_xts_blend$ARGO[zoo::index(our_argo$baidu$pred)] <- logit_inv(our_argo$baidu$pred[73:95])*100


pred_xts_blend$Santillana[zoo::index(santillana_etal$OBI$pred)] <- santillana_etal$OBI$pred[73:95]
...
zoom_periods <- c("2014-02-01/2014-12-01",
