...
#'
#' @export
plot_all3<- function(GFT_xts, GC_GT_cut_date, model_names, legend_names, zoom_periods){
  ...
  color_code <- c("red","orange", "forestgreen", "goldenrod3","blue2")
  term_change_col <- "khaki"
  cdc_lwd <- 2
  line_width <- c(1.5,1.5,2,2,2,2)
  line_type <- c(1,1,4,4,4,4)
  
  layout(rbind(rep(1,3),rep(2,3)), heights=c(2.5/3.5*5,6/3.5))
  par(mar=c(4.1, 2.1, 1.1, 1))
  xts::plot.xts(GFT_xts$Salesvolume, ylim=c(1000000,2500000),bar.col="yellow", type='p', auto.grid = FALSE, main=NA,pch=1)
  points(GFT_xts$ARGO,ylim=c(0.2,1.0), type='p', auto.grid = FALSE, main=NA,pch=2)
  points(GFT_xts$ARGO.With.Lag,ylim=c(0.2,1.0), type='p', auto.grid = FALSE, main=NA,pch=3)
  ...
  legend(xts::.index(GFT_xts)[1], 2500000,c("Salesvolume",legend_names),
  ...
  xts::plot.xts(GFT_xts$Salesvolume-GFT_xts[,model_names[i]], ylim=c(-1050000,800000),main=NA,typ='n',
