...
Saledata<-read_excel("C:/Users/Administrator/Desktop/数据/中国大陆汽车销量滞后4个月.xlsx")
...
end <- which(colnames(our_argo$baidu$coef)=="2016-01-01")
...
pred_xts_blend$ARGO[zoo::index(our_argo$baidu$pred)] <- logit_inv(our_argo$baidu$pred[1:110])*100


pred_xts_blend$Santillana[zoo::index(santillana_etal$OBI$pred)] <- santillana_etal$OBI$pred[1:110]
