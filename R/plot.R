...
#'
#' @export
plot_argo <- function(pred_xts_blend, argo_cut_date, model_names, legend_names, zoom_periods){
  par_orig <- par()[c("mar","mfrow")]
  cutting_dates <- which(index(pred_xts_blend) %in% c(argo_cut_date-94608001,argo_cut_date-1))
  ...
  xts::plot.xts(pred_xts_blend$销量数据, ylim=c(-0.1,8.5), type='n', auto.grid = FALSE, main=NA)

  abline(v=xts::.index(pred_xts_blend)[cutting_dates[1]], col=term_change_col, lwd=2, lty=4)
  rect(xts::.index(pred_xts_blend)[cutting_dates[2]], -10, xts::.index(pred_xts_blend)[nrow(pred_xts_blend)], 100,
       border=NA, col="lightgoldenrodyellow")
  box()
  lines(pred_xts_blend$销量数据, lwd=cdc_lwd)
  for(i in length(model_names):1){
    lines(pred_xts_blend[,model_names[i]], col=color_code[i], lwd=line_width[i], lty=line_type[i])
  }
  legend(xts::.index(pred_xts_blend)[73], 8.6,c("汽车销量",legend_names),
         col=c(1, color_code),
         lwd=c(cdc_lwd, line_width),
         lty=c(1,line_type), cex=1.5)

    #arrows(xts::.index(pred_xts_blend)[cutting_dates[1]-1],0,xts::.index(pred_xts_blend)[1],0,
    #      length=0.1, col="grey23", code=3)
    #text(xts::.index(pred_xts_blend)[cutting_dates[1]%/%2+1], 0,
    #     "Google Correlate", pos=3, offset=0.3, cex=1.25)
    #text(xts::.index(pred_xts_blend)[cutting_dates[1]%/%2+1], 0,
    #     "Search terms identified on 2009-03-28", pos=1, offset=0.3, cex=1.25)
    #arrows(xts::.index(pred_xts_blend)[cutting_dates[1]+1],0,xts::.index(pred_xts_blend)[cutting_dates[2]-1],0,
    #      length=0.1, col="grey23", code=3)
    #text(xts::.index(pred_xts_blend)[mean(cutting_dates)], 0,
    #     "Google Correlate", pos=3, offset=0.3, cex=1.25)
    #text(xts::.index(pred_xts_blend)[mean(cutting_dates)], 0,
    #     "Search terms identified on 2010-05-22", pos=1, offset=0.3, cex=1.25)
    #arrows(xts::.index(pred_xts_blend)[cutting_dates[2]+1],0,xts::.index(pred_xts_blend)[nrow(pred_xts_blend)-1],0,
    #       length=0.1, col="grey23", code=3)
    #text(xts::.index(pred_xts_blend)[(cutting_dates[2]+nrow(pred_xts_blend))%/%2+1], 0,
    #     "Google Trend", pos=3, offset=0.3, cex=1.25)
    #text(xts::.index(pred_xts_blend)[(cutting_dates[2]+nrow(pred_xts_blend))%/%2+1], 8,
    #     "Search terms\nidentified on\n2010-05-22", pos=1, offset=0, cex=1.25)

  old_mar <- par(mar=c(0.5,2.1,0.5,1))


  xts::plot.xts(pred_xts_blend$销量数据-pred_xts_blend[,model_names[i]], ylim=c(-2,2),main=NA,typ='n',
  ...
  for(i in length(model_names):1){
    lines(-pred_xts_blend$销量数据+pred_xts_blend[,model_names[i]], col=color_code[i],
  ...
  for(k in 1:length(zoom_id)){
    ...
    plot(pred_xts_blend$销量数据[plot_period], ylim=c(1.2,8.5),
         main=NA, type='n', auto.grid = FALSE)
    lines(pred_xts_blend$销量数据[plot_period], lwd=cdc_lwd)
    for(i in length(model_names):1){
      lines(pred_xts_blend[,model_names[i]][plot_period], col=color_code[i], lwd=line_width[i], lty=line_type[i])
    }
...
heatmap_argo <- function(argo_coef, lim){
  ...
  axis(1, at=seq(as.Date("2013-01-01"), as.Date("2016-06-01"), by="month"),labels=NA,
