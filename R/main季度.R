...
santillana_etal[["OBI"]] <-
  argo(y, exogen = exogen,
       alpha = 1, N_lag = NULL, use_all_previous = TRUE,  N_training = 48)  #不加时间序列

our_argo[["baidu"]] <-
  argo(logit(y / 100), exogen = log((exogen + 0.0005) / 100),
       alpha = 1, N_lag=1:24, use_all_previous = FALSE, N_training = 24)   #argo

ar3 <- argo(y, alpha = NA, use_all_previous = FALSE, N_lag=1:24,
            N_training = 48)  #无惩罚项，无百度指数，结果不稀疏
...
start <- which(colnames(our_argo$baidu$coef)=="2011-01-01")
end <- which(colnames(our_argo$baidu$coef)=="2016-05-01")
...
pred_xts_blend$ARGO[zoo::index(our_argo$baidu$pred)] <- logit_inv(our_argo$baidu$pred)*100


pred_xts_blend$Santillana[zoo::index(santillana_etal$OBI$pred)] <- santillana_etal$OBI$pred


pred_xts_blend <- pred_xts_blend["2011-01-01/"]
pred_xts_blend<-apply.quarterly(pred_xts_blend,mean)

model_names <- c("ARGO","Santillana","AR3","naive")
legend_names <- c("ARGO","Santillana_etal","AR(3)","Naive")

zoom_periods <- c("2011-01-01/2011-12-01",
...
plot_all(pred_xts_blend, argo_cut_date, model_names[1:4], legend_names[1:4], zoom_periods)
dev.off()
