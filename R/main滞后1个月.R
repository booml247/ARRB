...
Saledata<-read_excel("C:/Users/Administrator/Desktop/数据/中国大陆汽车销量滞后1个月.xlsx")
...
our_argo[["baidu"]] <-
  argo(logit(y / 100), exogen = log((exogen + 0.5) / 100),
       alpha = 1, N_lag=1:12, use_all_previous = FALSE, N_training = 13)   #argo
...
pred_xts_blend$ARGO[zoo::index(our_argo$baidu$pred)] <- logit_inv(our_argo$baidu$pred[1:113])*100


pred_xts_blend$Santillana[zoo::index(santillana_etal$OBI$pred)] <- santillana_etal$OBI$pred[1:113]
