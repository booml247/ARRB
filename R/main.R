...
# Real-case nowcast is available when dtname == "GT"
  ...
  our_argo[["baidu"]] <-
    argo(logit(y / 100), exogen = log((exogen + 0.5) / 100),
         alpha = 1, N_lag=1:24, use_all_previous = FALSE, N_training = 30)   #argo
...
start <- which(colnames(our_argo$baidu$coef)=="2012-01-01")
...
pred_xts_blend <- pred_xts_blend["2012-01-01/"]
